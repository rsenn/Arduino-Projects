...
template <typename Type> String TypeToPaddedString(Type Value, int PaddingCount) { // ugh, not ideal
...
String DoubleToString(double Value) { // ughhhh, really not ideal, but the MKR1000 STRING CONSTRUCTOR TAKING FLOATING
  ...
  if(decPart > 0)
    return (Return + '.' + decPart);
  else if(decPart < 0)
    return (Return + '.' + (-1) * decPart);
  else
    return (Return + '.' + 00);
}

bool
IsReadingValid(double Reading) {
...
double
MeasureVoltage(uint8_t FromPin, uint8_t ToPin) {
  return static_cast<double>((analogRead(AnalogReadMappings[FromPin]) - analogRead(AnalogReadMappings[ToPin]))) /
         MaxADCValue * RefVoltage;
}

double
MeasureCurrent(uint8_t FromPin, uint8_t ToPin) {
...
double
SetUpAndExclusivelyMeasureCurrent(uint8_t FromPin, uint8_t ToPin, uint8_t ExcludePin) {
