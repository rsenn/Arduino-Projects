 ...
 * WhistleSwitch.cpp
 *
 * Analyzes a microphone signal and toggles an output pin, if the main frequency is for a specified duration in a
 * specified range. For Tiny85 with 1 MHz
 *
 ...
 *  If the pitch is lower than the specified frequency the feedback LED blinks slowly, if the pitch is higher it blinks
 * fast. If the match holds for MATCH_TO_LONG_MILLIS (1.0 second) after switching output, the  output switches again to
 * go back to the former state. This can be useful if a machine generated signal (e.g. from a vacuum cleaner) matches
 * the range.
 *
 *  BUTTON OPERATION / RESET
 *  On button release, the relay state is toggled. If the press was in RELAY_DEAD_MILLIS (1) seconds after the last
 * press, the relay is NOT toggled. This relay dead time was introduced, to avoid relay switching while trying to
 * perform a reset by double press, see below. A reset can be performed by power off/on or by pressing the button two
 * times with each time shorter than RESET_ENTER_BUTTON_PUSH_MILLIS (0.12 seconds) within a RESET_WAIT_TIMEOUT_MILLIS
 * (0.3 seconds) interval. A button press longer than BUTTON_PUSH_ENTER_PROGRAM_SIMPLE_MILLIS (1.5 seconds) will enter
 * the programming mode, which is indicated by a LED blink.
 *
 *  INFO
 *  After power up or reset, the feedback LED echoes the range number. Range number 10 indicates an individual range,
 * programmed by advanced programming. The timeout state is signaled by short LED pulses after the range number feedback
 * (no short pulse -> no timeout enabled).
 *
 *  TIMEOUT
 *  After a timeout of TIMEOUT_RELAY_ON_SIGNAL_MINUTES_(1 or 2) (2 or 8 hours) the relay goes OFF for 1 second.
 *  In the next TIMEOUT_RELAY_SIGNAL_TO_OFF_MINUTES minutes you must then press the button or whistle the pitch to
 * cancel the timeout, otherwise the relay will switch OFF. Cancellation of timeout is acknowledged by the LED blinking
 * 5 times for 1 second on and off. Timeout can be switched on by selecting the dummy ranges 10 or 11 and off by
 * selecting the dummy ranges > 11. Default is TIMEOUT_RELAY_ON_SIGNAL_MINUTES_2 (8 hours).
 *
 *  PROGRAMMING
 *  Programming is done by a long press of the button.
 *  After BUTTON_PUSH_ENTER_PROGRAM_SIMPLE_MILLIS (1.5 seconds), the feedback LED blinks once for signaling simple
 * programming mode. After BUTTON_PUSH_ENTER_PROGRAM_ADVANCED_MILLIS (4 seconds), the feedback LED blinks twice for
 * signaling advanced programming mode. After releasing the button the corresponding programming mode is entered.
 *
 *  SIMPLE PROGRAMMING
 ...
 *  While you press the button the pitch range is measured. i.e. the minimum and maximum of the pitch you are whistling
 * is stored. No timeout here!
 *
 *  After button release, you may press the button again before the PROGRAM_MODE_ADVANCED_END_DETECT_MILLIS (3 seconds)
 * timeout. The duration of this second press is taken as the needed duration for the signal match to toggle the relay.
 ...
 *
 *  Since the internal oscillator is not very stable, use gaps of 20/10 Hz in order to avoid overlapping of ranges of 2
 * different devices PREDEFINED RANGES 1   1700 - 2050 Hz  -> 350 Hz 2   1500 - 1680 Hz  -> 180 Hz 3   1300 - 1480 Hz ->
 * 180 Hz 4   1150 - 1280 Hz  -> 130 Hz 5   1000 - 1140 Hz  -> 140 Hz 6    900 -  990 Hz  ->  90 Hz
 *
...
//#define TRACE
//#define DEBUG
// With INFO is enabled the program is too big for the Digispark board in the Arduino IDE, since it used the old
// bootloader size. If you want INFO on the ATtiny85 use the ATTiny85 from the ATTinyCore Board manager, since it leads
// to smaller code size, and upgrade the micronucleus with
// https://github.com/micronucleus/micronucleus/tree/master/upgrade/releases and:
// %UserProfile%\AppData\Local\Arduino15\packages\digistump\tools\micronucleus\2.0a4\launcher.exe -cdigispark
// -Uflash:w:upgrade-t85_default.hex:i this gives additional gives 500 bytes more. Then upload the hex file manually
// with: %UserProfile%\AppData\Local\Arduino15\packages\digistump\tools\micronucleus\2.0a4\launcher.exe -cdigispark
// -Uflash:w:<myprogram>.hex:i
//#define INFO
#else
...
//    BUTTON - PCINT3/XTAL1/CLKI/!OC1B/ADC3 (D3) PB3  2|    |7  PB2 (D2) SCK/USCK/SCL/ADC1/T0/INT0/PCINT2 - TX Debug
//    output
...
#define MAX_ACCEPTABLE_DELTA_FREQ                                                                                      \
  100 // max delta Freq for frequency change between two consecutive measurement in advanced programming mode. Used to
// detect dropouts.
...
#define MATCH_TO_LONG_MILLIS                                                                                           \
  1000                         // max milliseconds for match condition true, otherwise switch back to relay state before
...
#define TIMEOUT_RELAY_SIGNAL_TO_OFF_MINUTES                                                                            \
  3 // after signaling wait this time for feedback otherwise switch the relay OFF
...
#define RESET_WAIT_TIMEOUT_MILLIS                                                                                      \
  300 // milliseconds to push and release button the second time in order to perform reset command
/*
...
#define PROGRAM_MODE_TIMEOUT_MILLIS                                                                                    \
  5000 // milliseconds to enter regular detect mode if in program mode and no button press happens
#define PROGRAM_MODE_SIMPLE_END_DETECT_MILLIS                                                                          \
  1500 // milliseconds to enter regular detect mode if in simple program mode and a range has been chosen and no button
// press happens
#define PROGRAM_MODE_ADVANCED_END_DETECT_MILLIS                                                                        \
  3000 // milliseconds to enter regular detect mode if in advanced program mode and a range has been entered and no
...
struct WhistleSwitchControlStruct {
  ...
  int16_t MatchValidNeeded; // valid matches detected needed for accepting match, i.e. for toggling relay :=
  // MillisNeededForValidMatch/timeOfReading
...
struct ButtonControlStruct {
  ...
  /*
   * Duration is set at button release or from an outside loop which polls the button state in order to check for button
   * press timeouts, since we get no interrupt until button is released.
   */
 ...
 */
void
setFeedbackLedBlinkState(uint16_t aLedBlinkMillis, int8_t aLedBlinkCount) {
...
void
setMillisNeededForValidMatch(uint16_t aPeriodValidNeededMillis) {
...
setup() {
  ...
  /*
   * For Arduinos with other than optiboot bootloader wdt_disable() comes too late here, since after reset the watchdog
   * is still enabled and uses fastest prescaler value (approximately 15 ms)
   */
  wdt_disable();
#if defined(__AVR_ATtiny85__)
  initTXPin();
  ...
  for(;;) {
    ...
    if(WhistleSwitchControl.MainState == DETECT_FREQUENCY) {
      ...
      if(IS_BUTTON_ACTIVE) {
        ...
        handleButtonRelease(); // must be called before detectFrequency since detectFrequency introduces a big delay and
        // the button state may have changed
        detectFrequency();
      }
...
handleLongButtonPress() {
  ...
  if(ButtonControl.ButtonPressDurationMillis > BUTTON_PUSH_ENTER_PROGRAM_SIMPLE_MILLIS) {
    if(WhistleSwitchControl.ButtonPressTempState == DETECT_FREQUENCY) {
      ...
      Serial.println(F("Press the button once for range 1, twice for range 2 etc. Each button press is echoed by the "
                       "feedback LED."));
      ...
      Serial.println(F("Before the 5 seconds timeout, whistle the pitch you want to detect, then press the button "
                       "again to record."));
 ...
 * Problem: we are not sure that the level we read, is the level which triggers the interrupt, therefore do debouncing
 * here and then read compute active period duration
 */
//
#if defined(__AVR_ATtiny85__)
ISR(PCINT0_vect) {
...
#error Timer 0 overflow interrupt not set correctly
  ...
  if(IS_BUTTON_ACTIVE == ButtonControl.ButtonStateIsActive) {
    PCIFR = 1 << PCIF2; // Clear interrupt flag if no state change in order to cancel all interrupts generated by button
    // bouncing
  }
  PCICR = 1 << PCIE2; // PCINT2 enable
#ifdef MEASURE_TIMING
...
#elif defined(__AVR_ATtiny85__)
  if(IS_BUTTON_ACTIVE == ButtonControl.ButtonStateIsActive) {
    GIFR = 1 << PCIF; // Clear interrupt flag if no state change in order to cancel all interrupts generated by button
    // bouncing
  }
  GIMSK |= 1 << PCIE; // PCINT enable
#endif
