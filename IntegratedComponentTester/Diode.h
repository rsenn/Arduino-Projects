  ...
  DetectDiode() {
    for(size_t Index = 0; Index != ArrayCount(MutuallyExclusivePins); ++Index) {
      ...
      if(Readings.First) {
        ...
        Display::GetInstance().println(String("C: ") +
                                       (Readings.Fourth.First ? DoubleToString((Readings.Fourth.Second)) : "(?)") +
                                       static_cast<char>(229) + "F");
...
private:
  static Quadruple<bool, double, double, Pair<bool, double>>
  SetUpAndExclusivelyMeasureDiode(uint8_t FromPin, uint8_t ToPin, uint8_t ExcludePin) {
  ...
  static Quadruple<bool, double, double, Pair<bool, double>>
  MeasureDiode(uint8_t FromPin, uint8_t ToPin) {
